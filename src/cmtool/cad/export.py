...
def write_print_sheet(
    ...
    *,
    ...
    ``print_purpose`` is ``"trial"`` or ``"data"``. It appears on every record
    because the distinction is easy to lose afterwards and impossible to
    reconstruct: a trial print is one whose settings were still being adjusted,
    and its numbers must never end up in the dataset beside parts printed under a
    frozen recipe.
    """
    if print_purpose not in {"trial", "data"}:
        raise ValueError(f"print_purpose must be 'trial' or 'data', got {print_purpose!r}")
    out = Path(path)
    ...
    lines: list[str] = [
        ...
        "",
        f"**Print purpose: `{print_purpose}`** - "
        + (
            "settings are still being adjusted, so these numbers do not go in the dataset."
            if print_purpose == "trial"
            else "printed under a frozen recipe, so these numbers go in the dataset."
        ),
        "",
        "## Purpose",
        "",
    ...
    lines += [
        ...
        "",
        f"- [ ] Printer used: `{printer.name}` ({printer.model}, {printer.role}) - "
        "every record must name the printer",
        f"- [ ] Print purpose: `{print_purpose}` (trial or data)",
