...
> **Status: Phase A pilot, milestone A6 in progress.** Rigid kinematics, flexure sizing
> and feasibility, design search, the PRBM solver, the nonlinear beam FEA, printable CAD
> for coupons and mechanisms, the camera measurement pipeline, the HTML viewer and the
> figure scripts all work and are validated. What remains is physical: print, measure,
> film.
>
...
Every figure below is regenerated by one command, `cmtool figures`. **The measured series
is absent from them, not zero** — nothing has been printed yet. When it has, the same
command fills it in.

### The coupler path, four ways

![rigid, PRBM and beam FEA coupler paths, and how far apart they are](docs/figures/path_overlay.png)

The three predictions agree to within a millimetre across a part 140 mm wide, so the
left-hand panel is deliberately unremarkable: that is the finding. The right-hand panel is
where the difference lives. **The PRBM line sits exactly on zero** — with a prescribed
input and pivot-matched placement, stiffness cannot move a one-degree-of-freedom path at
all. Only the FEA, which lets the flexures stretch and shear, departs from it.

### Which is why torque, not path, tests the stiffness model

![PRBM and beam FEA input torque against input angle](docs/figures/torque_curves.png)

The two models differ by about half on peak torque — roughly 90 g against 135 g of hanging
weight at a 36 mm arm. That is far more than a set of kitchen weights struggles to resolve,
which is the whole argument for the dead-weight rig.

### Where the flexures are, in the browser

```bash
cmtool view examples/designs/fb_02_0052.json --html out/view.html
```

One self-contained file: no server, no install, no network. Open it on a phone at the
printer. A slider steps through precomputed FEA states, flexures coloured by strain, with
the rigid and PRBM outlines over them as ghosts and all the coupler paths underneath.

### The bound that governs the whole design space

![usable joint rotation against adjacent link length, for four flexure thicknesses](docs/figures/design_rule.png)

### An artefact that would otherwise look like physics
...
![gamma and K fitted across tip load ratios, against both Howell variants](docs/figures/prbm_variant.png)
...
src/cmtool/
  ...
  metrics/     torque templates, model comparison, path distance (pointwise + Frechet)
  vision/      ArUco layout, calibration, plane map, tracking, uncertainty
  viz/         scene model, self-contained HTML viewer, the figure scripts
