...
class TestStrainCoupon:
    """The mandrel bend test that measures the allowable strain."""

    def test_strain_uses_the_neutral_axis_not_the_thin_strip_limit(self):
        """Eps = (t/2)/(R + t/2), which the familiar t/(2R) overstates."""
        exact = bend_strain(0.6, 5.0)
        thin_limit = 0.6 / (2 * 5.0)
        assert exact == pytest.approx(0.3 / 5.3, rel=1e-12)
        assert exact < thin_limit
        assert thin_limit / exact == pytest.approx(1.06, abs=0.01)

    def test_strain_falls_with_radius(self):
        strains = [bend_strain(0.6, r) for r in StrainCouponSpec().mandrel_radii_mm]
        assert strains == sorted(strains, reverse=True)

    def test_mandrel_set_brackets_the_placeholder_allowable_strain(self):
        """The posts must span both sides of any plausible answer, or they cannot find it."""
        table = StrainCouponSpec().strain_table()
        values = [row["nominal_strain"] for row in table]
        assert min(values) < 0.010 < max(values)
        assert max(values) > 0.04

    def test_strips_are_upright_walls_so_they_bend_like_a_flexure(self):
        """Thickness in-plane, width out-of-plane: bending along extrusions, not across layers."""
        spec = StrainCouponSpec()
        extent = bounding_box_mm(build_strain_strips(spec))
        assert extent[2] == pytest.approx(spec.strip_height_mm)
        assert extent[0] >= spec.strip_length_mm

    def test_strips_are_separate_solids(self):
        spec = StrainCouponSpec()
        solid = build_strain_strips(spec)
        assert len(solid.vals()[0].Solids()) == spec.strip_count()

    def test_mandrel_block_fits_the_envelope(self, printer):
        extent = bounding_box_mm(build_strain_mandrels())
        envelope = printer.design_envelope_mm()
        assert extent[0] <= envelope[0] and extent[1] <= envelope[1]

    def test_data_template_has_a_row_per_combination(self):
        spec = StrainCouponSpec()
        text = strain_data_template(spec)
        rows = [line for line in text.splitlines() if line and not line.startswith("#")]
        assert len(rows) == 1 + len(spec.strain_table())
        assert "nominal_strain" in rows[0]
        assert "permanent_set_deg_after_10_cycles" in rows[0]

    def test_data_template_prefills_the_nominal_strains(self):
        text = strain_data_template(StrainCouponSpec())
        assert f"{bend_strain(0.6, 5.0):.5f}" in text


class TestCouponSet:
    def test_builds_every_coupon(self):
    ...
    def test_metadata_is_serialisable_and_complete(self):
        ...
        assert meta["strain_mandrels"]["strain_range"][0] < 0.01
        assert meta["strain_strips"]["strip_count"] == 6
